...
UAB0104 = Class(TStructureUnit) {   
    ...
    OpenState = State() {
        ...
        Main = function(self)            
            ...
            self.Trash:Add(CreateAttachedEmitter(self,'UAB2305',army, '/effects/emitters/structure_steam_ambient_01_emit.bp'):OffsetEmitter(0, 0, 0))
            self.Trash:Add(CreateAttachedEmitter(self,'UAB2305',army, '/effects/emitters/structure_steam_ambient_02_emit.bp'):OffsetEmitter(0, 0, 0))
            
            if newSat then
                self.Satellite = CreateUnitHPR('UASSP0100', self:GetArmy(), location[1], location[2], location[3], 0, 0, 0)
                self.Trash:Add(self.Satellite)
                self.Satellite:AttachTo(self, 'Attachpoint')
            end
            ...
            WaitFor( self.AnimManip )
			self.Trash:Add(CreateAttachedEmitter(self,'UAB2305',army, '/effects/emitters/uef_orbital_death_laser_launch_01_emit.bp'):OffsetEmitter(0.00, 0.00, 0.00))
			self.Trash:Add(CreateAttachedEmitter(self,'UAB2305',army, '/effects/emitters/uef_orbital_death_laser_launch_02_emit.bp'):OffsetEmitter(0.00, 0.00, 0.00))
